...
define(function (require) {
    ...
    function domReady() {
        ...
        $.ajax($.extend({
            url: apiHost + '/hoss/league/contract/modyOpen.do',
            data: {id: id,contractId:contractId}
                ...
                function useful(data) {
                    if (data.status == 1) {
                        ...
                        //$('#modifyLink').attr("href","modify.html?companyId="+dataObj.id+"&contractId="+dataObj.contractId+"&userId="+dataObj.userId);
                    } else {
        ...
        $("#modifybtn").click(function(event){
            ...
            $.ajax({
                type: "POST",
                url : apiHost + "/hoss/league/contract/modifyLeagueAdmin.do",
                dataType : 'json',
                ...
                if (json.detail.indexOf("成功") == -1 ) {
                    systemMessage({
                        type: 'info',
                        title: '提示：',
                        detail: json.detail || '修改失败！'
                    });
                } else {
                    systemMessage({
                        type: 'info',
                        title: '提示：',
                        detail: json.detail || '修改成功！'
                    });
