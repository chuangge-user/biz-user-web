define(function(require, exports, module) {
    ...
    $(function () {
                ...
                .done(function (data) {
                    ...
                    function useful(data) {
                        if ('liqu' == type) {
                            ...
                            if(data.data != 'red'){
                                $('.qd-liqu').attr("href",webHost+'/app/approval/wait-apply/brokerage-modal-view.html?businessKey='+data.data.split(":")[2]
                                    +'&wfInstanceId='+data.data.split(":")[1]+'&projectId='+$projectId );
                                $('.qd-liqu').attr("title", "查看佣金设置");
                            }
                        }
                        if ('channel' == type) {
                            $('.qd-manager > i').attr("class", iconClass[data.data || 'orange']);
                        }
                    }
                    ...
                    function useless(data) {
                        ...
                        if ("1" != data.status && 1 != data.status) {
                            ...
                            if (-1 == data.data.id || "-1" == data.data.id) {
                                ...
                                $('.qd-yd-budget').attr("title", '请完成月度预算');
                            } else {
                                ...
                                $('.qd-yd-budget').attr("title", '查看月度预算');
                            }
                        }
                    }
                    doneCallback.call(this, data, useless);
                    ...
                    function useless(data) {
                        ...
                        if ("1" != data.status && 1 != data.status) {
                            ...
                            if (-1 == data.data.id || "-1" == data.data.id) {
                                ...
                                $('.qd-ys-budget').attr("title", '请完成全周期预算');
                            } else {
                                ...
                                $('.qd-ys-budget').attr("title", '查看全周期预算');
                            }
                        }
                    }
        ...
        function initUrl(){
            ...
            $('#qd-manager').attr("title", "中介渠道管理");
            $('.qd-liqu').attr("href", "../approval/new-apply/brokerage-modal-apply.html?project=" + $projectId);
            $('.qd-liqu').attr("title", "请进行佣金设置");
        ...
        $searchFormRemind.on('submit', function () {
            ...
            $.ajax($.extend({
                ...
                beforeSend: function () {
                    messageType = $('#formControlSmSelect').val();
                }
            }, jsonp))
                .done(function (data) {
                    function useless(data) {
                        var dataObj = data.data || {},
                            dataContent = dataObj.content,
                            dataFilter = [];
                        if ( +messageType !== 0 ) {
                            dataContent.forEach(function(v){
                                if ( v.msgType == messageType ) {
                                    dataFilter.push(v);
                                }
                            });
                            data.data.content = dataFilter;
                        }
