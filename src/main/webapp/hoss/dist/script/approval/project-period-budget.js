define(function (require) {
    ...
    var popover = require('bootstrap/popover'); // 提示窗口组件
    ...
    $(document).ready(function () {
        ...
        if (first_project !='' || first_project != undefined) flag = 'ecbk';
            ...
            if(null!=getQueryString('cityId')&&"null"!=getQueryString('cityId')&&""!=getQueryString('cityId')&&"Null"!=getQueryString('cityId')){
                ...
                $("#cityId").trigger('change', function () {
                    ...
                    if(null!=first_project&&"null"!=first_project&&""!=first_project&&"Null"!=first_project){
                        getProjectFullInfo(first_project);
                        first_project = null;
                    }
                });
            }
            if(""!=$project.val()&&!isNaN($project.val())&&-1!=$project.val()){
                getProjectFullInfo($project.val())
            }
        }, flag);

        function getProjectFullInfo(obj){
            $.ajax($.extend({
                url: apiHost + '/hoss/sys/budget/findFullBudgetByProjectId.do',
                data: {projectId: obj}
            }, jsonp))
                .done(function (data) {
                    doneCallback.call(
                        ...
                        function (data) {
                            if("1"!=data.status&&1!=data.status){
                                ...
                                initFlod(data);
                                $("#versionsPlan").text(data.data.versions_plan);
                                $("#username").text(data.data.project_user_name)
                                $("#projectTime").text(data.data.start_date+"------"+data.data.end_date)
                                checkFirst();
                            }
                        }
                    );
